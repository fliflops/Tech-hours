...
const Details = (props) => {
    const {index,id,project_code,emp_id,actual_date,actual_duration,sequence_no,remarks,adhoc} = props;
    ...
    const handleSave = () => {
        const {remarks,adhoc,...required} = state
        ...
        .then(result => {
            if(result.status === 200){
                ...
                props.updateData(index, 'adhoc', state.adhoc)
            }
        })      
    }
    ...
    return (
        <Grid component={'div'} className='container' container>
            ...
            <Grid item container xs={12}>
                <Numeric    size={1} label='Seq. No'            name='sequence_no'          value={state.sequence_no}       handleChange={handleChange} isLabelVisible/>
                <DatePicker size={3} label='Actual Date'        name='actual_date'          value={state.actual_date}       handleChange={handleChange} isLabelVisible/>
                <Numeric    size={2} label='Actual Duration'    name='actual_duration'      value={state.actual_duration}   handleChange={handleChange} isLabelVisible/>
                <Input      size={3} label='Remarks'            name='remarks'              value={state.remarks}           handleChange={handleChange} isLabelVisible/>
                <Input      size={3} label ='Adhoc'             name= 'adhoc'               value={state.adhoc}             handleChange={handleChange} isLabelVisible/>
            </Grid>    
        </Grid>    
    )
}
